...
resource "azurerm_role_assignment" "alerthandler_kv_keyvaultsecretsuser" {
  scope                = azurerm_key_vault.nbs_appts_kv.id
  role_definition_name = "Key Vault Secrets User"
  principal_id         = azurerm_linux_function_app.nbs_alerthandler_func.identity.0.principal_id
}
